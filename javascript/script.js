/* ============================================================
   POCKET APP — script.js  (Daily Survival Budget Ally v2)
   Dynamic Add Cash · Bridge (Utang) protocol · Savings Fund · Health tracking
   ============================================================ */

/* ── Supabase Cloud Integration (Pillar 2 & 3) ── */
...
/* ── Cached Supabase session (stays synchronous for the rest of the code) ── */
let _supaSession = null;
let _supaUser = null;
supabase.auth.onAuthStateChange((_evt, session) => { _supaSession = session; _supaUser = session?.user || null; });
supabase.auth.getSession().then(({ data }) => { _supaSession = data.session; _supaUser = data.session?.user || null; });

/* ── Cloud sync helper (debounced push after each save) ── */
let _syncTimer = null;
function syncToCloud(state) {
  if (!_supaSession || !_supaUser) return; // not logged in — silent no-op
  if (_syncTimer) clearTimeout(_syncTimer);
  _syncTimer = setTimeout(async () => {
    try {
      const uid = _supaUser.id;
      await Promise.all([
        supabase.from('profiles').upsert({ id: uid, cash_on_hand: state.cashOnHand, emergency_vault: state.emergencyVault, next_income_date: state.nextIncomeDate, setup_date: state.setupDate, updated_at: new Date().toISOString() }),
        state.transactions.length ? supabase.from('transactions').upsert(state.transactions.map(tx => ({ id: tx.id, user_id: uid, date: tx.date, amount: tx.amount, tier: tx.tier || 2, category: tx.category || null, note: tx.note || null, satiety_score: tx.satietyScore || null, paid_via_utang: tx.paidViaUtang || false, linked_utang_id: tx.linkedUtangId || null, is_vault_withdraw: tx.isVaultWithdraw || false, vault_amount: tx.vaultAmount || null, vault_reason: tx.vaultReason || null })), { onConflict: 'id' }) : Promise.resolve(),
        state.tier1Bills.length ? supabase.from('bills').upsert(state.tier1Bills.map(b => ({ id: b.id, user_id: uid, label: b.label, amount: b.amount, due_date: b.dueDate, is_paid: b.isPaid || false })), { onConflict: 'id' }) : Promise.resolve(),
        state.utangLedger.length ? supabase.from('utang_ledger').upsert(state.utangLedger.map(u => ({ id: u.id, user_id: uid, amount: u.amount, label: u.label, date: u.date, is_paid: u.isPaid || false })), { onConflict: 'id' }) : Promise.resolve(),
        state.microIncomeLedger.length ? supabase.from('income_ledger').upsert(state.microIncomeLedger.map(i => ({ id: i.id, user_id: uid, amount: i.amount, label: i.label, date: i.date })), { onConflict: 'id' }) : Promise.resolve(),
      ]);
      console.log('%c[Sync] Cloud sync complete ✓', 'color:#58cc02; font-weight:bold;');
    } catch (err) { console.warn('[Sync] Cloud sync failed:', err.message); }
  }, 3000);
}
...
function saveState(state) {
  ...
  syncToCloud(state); // Pillar 2: auto-sync to Supabase if logged in
}
...
/* ════════════════════════════════════════════════════════════
   REAL SUPABASE AUTH (replaced mock — Pillar 3)
   Same API surface: isLoggedIn(), getUser(), sendOTP(), verifyOTP(), etc.
   ════════════════════════════════════════════════════════════ */
...
const AuthManager = {
  ...
  /* ── Synchronous state checks (uses cached _supaSession from top of file) ── */
  isLoggedIn() { return !!_supaSession; },

  getUser() {
    if (!_supaUser) return null;
    return {
      id: _supaUser.id,
      name: _supaUser.user_metadata?.full_name || 'Pocket User',
      identifier: _supaUser.email || _supaUser.phone || '',
      provider: _supaUser.app_metadata?.provider || 'email',
      avatarEmoji: '👤',
    };
  },
  ...
  async verifyOTP(code) {
    const { data, error } = await supabase.auth.verifyOtp({ email: this._otpEmail, token: code, type: 'email' });
    if (error) return { success: false, message: error.message };
    return { success: true, token: data.session?.access_token, user: this.getUser() };
  },
  ...
  async logout() {
    ...
    _supaSession = null; _supaUser = null;
  },

  initCaptcha() { return Promise.resolve({ token: 'supabase_managed' }); },

  /* ── Nudge compatibility ── */
  _loadAuth() {
    return { isLoggedIn: this.isLoggedIn(), nudgeDismissed: !!localStorage.getItem('pocket-nudge-dismissed') };
  },
  _saveAuth() { /* Supabase manages its own session */ },
};
