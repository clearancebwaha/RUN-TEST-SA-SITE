/* ============================================================
   ...
   Dynamic micro-income · Utang protocol · Buffer tax · Health tracking
   ============================================================ */
...
const INITIAL_STATE = {
  ...
  emergencyVault   : 0,       // centavos — locked Tier 3 savings
  nutritionLog     : [],      // [{ date, score, cost }]
  lastProcessedDate: null,    // ISO — last date we ran end-of-day rollover
};
...
/* ════════════════════════════════════════════════════════════
   4. BUDGET ENGINE — Dynamic micro-income + 50/50 buffer tax
   ════════════════════════════════════════════════════════════ */

/**
 * processEndOfDay(state)
 * Called on boot / after actions. Walks from lastProcessedDate to yesterday,
 * applying the 50/50 buffer tax on each day's surplus.
 * Mutates state.emergencyVault and state.lastProcessedDate, returns rollover.
 */
function processEndOfDay(state) {
  ...
  const availableCash = state.cashOnHand - billsTotal - utangOwed;
  ...
  for (let d = new Date(start); d < end; d.setDate(d.getDate() + 1)) {
    ...
    if (surplus > 0) {
      // 50/50 BUFFER TAX: half rolls over, half locked in vault
...
function computeEngine(state) {
  ...
  /* ── Process past days (50/50 buffer tax) ── */
...
function toggleBillPaid(id) {
  ...
  if (bill) { bill.isPaid = !bill.isPaid; saveState(appState); }
}
  ...
  el.innerHTML = `
    <div class="space-y-3">
      <div style="display:flex; justify-content:space-between; align-items:center;">
        <span class="text-sm font-bold" style="color:#9a9a9a;">Initial Funds</span>
      ...
      <div style="display:flex; justify-content:space-between; align-items:center;">
        <span class="text-sm font-bold" style="color:#9a9a9a;">Days Until Next Income</span>
        <span class="font-nums text-sm" style="color:#9a9a9a;">${daysToNext} days</span>
      </div>
      ...
      <div class="text-xs font-semibold text-center" style="color:#9a9a9a;">
        ...
        ${isDeficit ? 'Consider the Crisis Protocol to bridge the gap 🛡️' : `${formatPeso(Math.max(0, daily))}/day for ${daysToNext} days`}
      </div>
    </div>`;
}
...
/* ── Deficit Alert + Crisis Protocol trigger ── */
  ...
  el.innerHTML = `
    <div class="rounded-2xl p-4 relative overflow-hidden animate-shake"
      ...
      <div style="position:relative; display:flex; align-items:center; gap:12px;">
        ...
        <div style="flex:1;">
          <div class="font-display" style="font-size:14px; color:#8a6d00;">Crisis Protocol Available</div>
          <div style="font-size:11px; font-weight:700; color:#9a9a9a; margin-top:2px;">
            You're ₱${(deficitAmt/100).toFixed(0)} short. Activate the bridge to keep moving forward.
          </div>
        </div>
    ...
    <div class="rounded-2xl p-4 animate-fade-in"
      ...
      <div style="display:flex; align-items:center; justify-content:space-between;">
        ...
        <div style="text-align:right;">
          <div style="font-size:10px; font-weight:700; color:#9a9a9a;">NEXT INCOME</div>
          <div style="font-size:13px; font-weight:700; color:#2e2e2e;">${nextLabel}</div>
          <div style="font-size:10px; font-weight:600; color:#9a9a9a;">${engine.daysToNext} day${engine.daysToNext!==1?'s':''} left</div>
        </div>
      </div>
    </div>`;
}
...
/* ── Buffer Fund (rollover portion only) ── */
    ...
    <div class="card-green-glow rounded-2xl p-4 relative overflow-hidden animate-fade-in">
      ...
      <div style="position:relative; display:flex; align-items:center; justify-content:space-between;">
        <div style="display:flex; align-items:center; gap:12px;">
          ...
          <div>
            <div class="font-display" style="font-size:15px; color:#2e2e2e;">Rollover Buffer</div>
            <div style="font-size:11px; font-weight:700; color:#58cc02;">50% of your savings ✓</div>
          </div>
        </div>
        <div style="text-align:right;">
...
/* ── Emergency Vault (Tier 3 — locked savings) ── */
      ...
      <div style="position:relative; display:flex; align-items:center; justify-content:space-between;">
        <div style="display:flex; align-items:center; gap:12px;">
          ...
          <div>
            ...
            <div style="font-size:11px; font-weight:700; color:rgba(255,215,0,0.7);">Tier 3 • Locked & protected 🔒</div>
          </div>
        </div>
        <div style="text-align:right;">
    ...
    <div class="rounded-2xl p-4 animate-fade-in"
      ...
      <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:8px;">
        <div style="display:flex; align-items:center; gap:8px;">
          ...
          <span class="font-display" style="font-size:15px; color:#2e2e2e;">Promissory Notes</span>
        </div>
      ...
      ${active.length > 0 ? `
        ...
        <div style="font-size:11px; font-weight:600; color:#9a9a9a;">Auto-deducts from your next income 🔄</div>
      ` : `<div style="font-size:12px; font-weight:600; color:#58cc02;">You cleared ${paid.length} note${paid.length!==1?'s':''}! 🎉</div>`}
    </div>`;
}
  ...
  el.innerHTML = `
    <div class="rounded-2xl p-4 animate-fade-in"
      ...
      <div style="display:flex; align-items:center; gap:12px;">
        ...
        <div>
          <div class="font-display" style="font-size:14px; color:#cc5500;">Body Investment Alert</div>
          <div style="font-size:11px; font-weight:700; color:#9a9a9a; margin-top:2px;">
          ...
          return `<button class="btn-squishy vault-bill-btn" data-bill-id="${bill.id}"
            ...
            <span style="display:flex; align-items:center; gap:10px;">
              ...
              <span style="font-size:10px; font-weight:700; border-radius:8px; padding:3px 8px;
                           background:rgba(255,255,255,0.08); color:#9a9a9a;">${formatDate(bill.dueDate)}</span>
            </span>
          </button>`;
        }).join('')}
      </div>` : ''}
...
function setupEventListeners() {
  ...
  document.getElementById('export-btn').addEventListener('click', () => { exportJSON(); closeModal('modal-settings'); });
  ...
  /* ── Add Funds modal ── */
  document.getElementById('add-funds-btn').addEventListener('click', () => openModal('modal-addfunds'));
...
function submitUtang() {
  ...
  showToast(`🛡️ Bridge activated! ₱${(amount/100).toFixed(0)} secured. Auto-pays on next income.`);
  renderDashboard();
}
